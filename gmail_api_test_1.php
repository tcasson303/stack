...
if (!isset($_SESSION['ACCESS_TOKEN'])) {
  ...
  echo '</ul>';
  echo 'Search for Deals: <ul>';
  $q = $_GET['query'];
  if ($q == "") {
    $q = "Deal";
  }
  //echo 'Searching for Senders matching "' . $q . "\":";
  echo 'Printing 20 most recent messages:';
  $searched = $imap->search(array("FROM", $q));
  //print_r($searched);
  foreach ($searched as $j => $value) {
  $mfile = $storage->getUniqueId($searched[$j]);
  $mbody = $storage->getMessage($searched[$j])->getContent();
  file_put_contents("./users/$newdir/$mfile.html", "$mbody");
  $imgfile[$j] = "./users/$newdir/$mfile-0.png";
  if (!file_exists($imgfile[$j])) {
    $convertout = `/opt/wkhtmltopdf/bin/wkhtmltopdf ./users/$newdir/$mfile.html ./users/$newdir/$mfile.pdf; convert -quality 95 ./users/$newdir/$mfile.pdf -resize 50% ./users/$newdir/$mfile.png`;
  }
  echo '<div id="search' . $j . '" style="{border:1px solid blue}"><img src="' . $imgfile[$j] . '"></div>' . "\n";
